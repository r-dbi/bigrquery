...
#' @S3method do grouped_bigquery
#' @importFrom dplyr do
do.grouped_bigquery <- function(.data, .f, ..., .chunk_size = 1e5L) {
  ...
  select <- select_query(
    from = .data$table,
    select = c(trans_bigquery(.data$group_by), vars),
    where = trans_bigquery(.data$filter),
    order_by = c(var_names(.data$group_by), trans_bigquery(.data$arrange)))
